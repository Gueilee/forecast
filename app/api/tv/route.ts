...
export async function GET() {
  ...
  const entities: TvEntity[] = entityList.map(entity => {
    ...
    const yearE = budgetEntries.filter(e => e.client.entity === entity)
  ...
  const allClients: TvClient[] = clients.map(client => {
  ...
  const marginYtd = actuals
    .filter(a => a.month <= currentMonth)
    .reduce((s, a) => s + (a.marginLiquid ?? 0), 0)
