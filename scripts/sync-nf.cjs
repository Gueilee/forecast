...
// filial placeholder (VB_ANALISE_NF01 não expõe FIL_COD)
const FILIAL_PLACEHOLDER = 0
...
async function buildClientMap(turso, oConn) {
  // Load clients from Turso
  const { rows: dbClients } = await turso.execute(
    `SELECT id, name, "nameReduced", "conexosCode", "conexosName" FROM "Client"`
  )

  // Check which ones already have conexosCode
  const alreadyMapped = dbClients.filter(c => c.conexosCode != null).length
  if (alreadyMapped > 0) {
    const map = {}
    dbClients.filter(c => c.conexosCode != null)
      .forEach(c => { map[Number(c.conexosCode)] = String(c.id) })
    console.log(`📋 ${alreadyMapped} clientes já mapeados`)
    return map
  }
  ...
  for (const cc of cnxClients) {
    ...
    // Try exact match on name or nameReduced
  ...
  if (updates.length > 0) {
    ...
    console.log(`📋 0 clientes mapeados automaticamente — NFs serão salvas sem clientId`)
  }
...
async function main() {
  console.log(`\nSincronizando ${periods.length} período(s)...\n`)
  ...
  for (const { year, month } of periods) {
    ...
    const { rows } = await oConn.execute(`
      SELECT DOC_COD, FIS_NUM_DOCUMENTO, COD_CLIENTE, CLIENTE,
             FIL_DES_FANTA, UND_NEGOCIO, TIPO_NF, TIPO_PROCESSO,
             PROC_CNX, REF_CLIENTE, FIS_DTA_EMISSAO,
             TOT_LIQUIDO, TOT_PRODUTOS, TOT_CUSTO_MEDIO,
             VLR_MNY_ICMS, VLR_MNY_ICMSST, VLR_MNY_PIS,
             VLR_MNY_COFINS, VLR_MNY_IPI, VLR_FRETE
      ...
      for (const r of batch) {
        ...
        stmts.push({
          ...
          args: [
            ...
            String(r.CLIENTE || ''), r.UND_NEGOCIO ? String(r.UND_NEGOCIO) : null,
            FILIAL_PLACEHOLDER, docId, emIso,
            year, month, wom,
        ...
        // Weekly aggregation
        const wKey = `${clientId || '_'}|${year}|${month}|${wom}`
        if (!weeklyMap[wKey]) {
    ...
    // Upsert ActualWeekly (only linked clients)
  ...
  await turso.execute({
    ...
    args: [totalNew, totalUpdated, totalNew + totalUpdated, new Date().toISOString(), jobId],
  })

  console.log(`\n✅ Concluído! Novas: ${totalNew} | Atualizadas: ${totalUpdated}`)
  await oConn.close()
  process.exit(0)
}
